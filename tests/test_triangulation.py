...
class TestTriangulation(SceneFixture):
    def test_triangulate_function(self):
        ...
        point_3d = triangulate(start_points0, dir_vec0[:, np.newaxis], start_points1, dir_vec1[:, np.newaxis])
        np.testing.assert_array_equal(point_3d.ravel(), np.array([25 / 4.0, 25 / 4.0, 9 / 2.0]))

    def test_given_cooresponding_points_in_image_frame_and_camera_extrinsic_then_3d_point_in_world_can_be_triangulated(
        self,
    ):
