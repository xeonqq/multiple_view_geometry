...
from multiple_view_geometry.algorithm import solve_essential_matrix, calculate_essential_matrix, reconstruct_translation_and_rotation_from_svd_of_essential_matrix, structure_from_motion
...
class TestEightPointAlgorithm(SceneFixture):
    def test_eight_point_algorithm(self):
        ...
        points_3d_in_camera1_frame, points_3d_scale = structure_from_motion( \
        ...
        tranlsation_cam1_wrt_cam0, rotation_cam1_wrt_cam0 = (T1, R1) if (points_3d_scale is not None and (points_3d_scale > 0).all()) else (T2, R2)
        scale = 3./5.
